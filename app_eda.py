...
class Home:
    def __init__(self, login_page, register_page, findpw_page):
        ...
        **Population Trends 데이터셋**  
        - 출처: 국내 통계청 또는 교육 목적 데이터 (CSV 파일)  
        - 설명: 연도별·지역별 인구, 출생자 수, 사망자 수 등의 변화를 담은 통계 데이터  
        - 주요 컬럼:  
          - `연도 (year)`: 인구 통계가 기록된 기준 연도  
          - `지역 (region)`: 행정구역 단위 (예: Seoul, Busan 등)  
          - `인구 (population)`: 해당 지역의 전체 인구 수  
          - `출생아수(명) (births)`: 해당 연도에 태어난 출생아 수  
          - `사망자수(명) (deaths)`: 해당 연도에 사망한 인구 수  
        - 활용 목적:  
          - 연도별 인구 추이 분석  
          - 지역 간 인구 변화 비교  
          - 인구 예측 및 시각화 기반 통계 해석
        """)
